...
controls = [
    ...
    dict(id_=30,  ccid='CC6.2',  crlg='CRLG#30', cat='cc6', title='AWS Prod Access (IAM + SSO)',           desc='AWS user list and permissions by role. SSO configuration enabled. All production access via Okta SSO/SAML.',                                                                            badge='auto',   tools=[], task='Review AWS IAM context for MFA status and access keys. Cross-reference with Okta context for SSO assignments. Confirm all production access is via SSO/SAML. Flag any users without MFA or with overly permissive policies.'),
    dict(id_=31,  ccid='CC6.3',  crlg='CRLG#31', cat='cc6', title='SSO / Password Policy',               desc='Policies of SSO to company programs. Screenshot of password policy settings in Okta (MFA, session timeout, complexity).',                                                                badge='auto',   tools=[], task='Review Okta context for password policy and MFA enforcement settings. Confirm: minimum password length >= 12, MFA required for all admins, session timeout policies are in place. Flag any gaps in policy enforcement.'),
    dict(id_=32,  ccid='CC6.4',  crlg='CRLG#32', cat='cc6', title='Admin Access List',                   desc='A list of Okta admins, AWS admins, and Salesforce admins with business justification.',                                                                                          badge='auto',   tools=[], task='Review Okta, AWS IAM, and Salesforce contexts for admin accounts. For each admin: verify business justification, confirm MFA enabled, and check last access from CloudTrail/audit logs. Flag any admins not accessed in 90+ days or lacking MFA.'),
    ...
    dict(id_=34,  ccid='CC6.6',  crlg='CRLG#34', cat='cc6', title='GitHub / Source Control Access',      desc='List of users with access to GitHub. SSO enabled for all GitHub access via Okta.',                                                                                                        badge='auto',   tools=[], task='Review GitHub context for organization members and repository access. Flag all outside collaborators (🚨) and verify each has documented business justification. Confirm all access follows least-privilege principle.'),
    ...
    dict(id_=45,  ccid='CC7.6', crlg='CRLG#45', cat='cc7', title='Root Cause Analysis',             desc='Evidence of change requests created following high severity incidents, including root cause analysis reviewed by operations management.',                                                    badge='auto', tools=[], task='Review Salesforce Service Cloud and incident tracking for high-severity incidents during audit period. For each incident: confirm RCA documented, corrective action assigned, and remediation tracked to completion. Flag incidents without documented analysis.'),
    # CC8
