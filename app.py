...
def _vet_vendor_impl():
    ...
    report_resp = client.messages.create(
        ...
        messages=[{"role": "user", "content":
            ...
            f"REQUIRED FORMAT FOR EACH CRITERION:\n"
            f"[Criterion Number]. [Criterion Name] — [✅/⚠️/❌]\n"
            f"Evidence: [Specific quote or finding - what did you find?]\n"
            f"Source: [Which page/document? Include URL if from web, or 'UPLOADED DOCUMENT' if from file]\n"
            f"Confidence: [HIGH/MEDIUM/LOW - based on clarity and # of sources confirming this]\n"
            f"[If ❌ REJECTED, add: Recommendation: [What vendor needs to provide/fix]]\n\n"
            f"🔐 SECURITY\n"
    ...
    # Add legal disclaimer at the top of report
    disclaimer = """
ASSESSMENT DISCLAIMER
====================
This vendor security assessment is generated by AI analysis and is NOT a substitute
for professional legal or security review. It is based on publicly available
information and vendor-provided documents only. Coralogix makes no warranties
about the accuracy or completeness of this assessment.

Use this assessment to inform procurement decisions. Obtain professional
legal review for HIGH RISK vendors before signing contracts.
====================
"""
    report = disclaimer + "\n\n" + report
    ...
    # Save vetting record with audit trail
    try:
        import time
        assessment_start_time = time.time()  # Approximate start time
